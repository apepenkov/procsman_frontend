...
function ProcessCardEdit({process, selectedTab, showDetails, setShowDetails}) {
    ...
    const refreshFromProcess = () => {
        ...
        setAutoRestartOnCrash(process.getConfiguration('auto_auto_restart_on_crash'));
        setAutoRestartMaxRetries(process.getConfiguration('auto_restart_max_retries'));
        setAutoRestartMaxRetriesFrame(process.getConfiguration('auto_restart_max_retries_frame'));
    ...
    const editProcess = (do_ = false) => {
        ...
        const maxRetriesErr = api.validate("configuration", "auto_restart_max_retries", patchProcessParams.config.auto_restart_max_retries);
        if (maxRetriesErr !== null) {
            errors.processRestartMaxRetries = maxRetriesErr;
        }

        const maxRetriesFrameErr = api.validate("configuration", "auto_restart_max_retries_frame", patchProcessParams.config.auto_restart_max_retries_frame);
        if (maxRetriesFrameErr !== null) {
            errors.processRestartTimeFrame = maxRetriesFrameErr;
        }

        const maxRetriesDelayErr = api.validate("configuration", "auto_restart_delay", patchProcessParams.config.auto_restart_delay);
        ...
        if (
            auto_restart_max_retries_within_timeframe < patchProcessParams.config.auto_restart_max_retries
        ) {
            if (!wasWarned) {
