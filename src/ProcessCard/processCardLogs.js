...
function ProcessCardLogs({process, selectedTab, showDetails}) {
    ...
    const [dateTimeFrom, setDateTimeFrom] = useState(new Date(new Date().setHours(new Date().getHours() - 4)));
    ...
    const [convertedLogs, setConvertedLogs] = useState(convert.toHtml('\x1b[30mblack\x1b[37mwhite'))
    ...
    const loadLogs = async () => {
        const logsResponse = await api.getProcessLogs(process.id, dateTimeFrom, dateTimeTo)
    ...
    useEffect(() => {
        ...
        const lines = convertedLogs.split('\n').map(line => {
            ...
            const highlightedLine = parts.map(part => {
                ...
                if (part.startsWith('<') && part.endsWith('>')) {
                    ...
                    return part.replace(searchRegex, match => {
                        const highlightClass = globalMatchIndex === currentMatchIndex ? 'current-highlight' : 'highlight';
    ...
    useEffect(() => {
        ...
        setCurrentMatchIndex(prevIndex => prevIndex >= matchCount ? 0 : prevIndex); // Reset if out of bounds
    ...
    return <Container>
        <Form.Control as="textarea" placeholder={"> Text to send to stdin"} onChange={e => {
        ...
        <Container>
            <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '10px'}}>
                <div>
                    <FormGroup>
                        ...
                        <Button variant="outline-secondary"
                                ...
                                onClick={() => setCurrentMatchIndex(currentMatchIndex - 1 >= 0 ? currentMatchIndex - 1 : totalMatches - 1)}
                                style={{width: 'auto', display: 'inline-block', verticalAlign: 'middle'}}
                        ><ArrowLeft style={{marginBottom: '5px'}}></ArrowLeft></Button>
                        <Button variant="outline-secondary"
                                disabled={totalMatches === 0}
                                onClick={() => setCurrentMatchIndex((currentMatchIndex + 1) % totalMatches)}
                                style={{width: 'auto', display: 'inline-block', verticalAlign: 'middle'}}
                        ><ArrowRight style={{marginBottom: '5px'}}></ArrowRight></Button>
                        <Form.Check
                            type="checkbox"
                ...
                <Button variant="outline-secondary" onClick={() => setIsFull(!isFull)}>
                    {isFull ? <><FullscreenExit size={24}/> Collapse</> : <><Fullscreen size={24}/> Expand</>}
                </Button>
            </div>

            <div className={`log-container ${isFull ? 'full' : 'collapsed'}`}
                 dangerouslySetInnerHTML={processedLogs}></div>
        </Container>
